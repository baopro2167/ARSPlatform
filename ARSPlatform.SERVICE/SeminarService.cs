...
namespace ARSPlatform.SERVICES
{
    public class SeminarService : ISeminarService
    {
        ...
        public SeminarService(ISeminarRepository seminarRepository, ISeminarParticipantRepository participantRepository, IUserRepository userRepository, IGoogleMeetService googleMeetService, IEmailService emailService, IMapper mapper, ILogger<SeminarService> logger)
        {
        ...
        public async Task<SeminarResponse?> GetByIdAsync(int seminarId, int? organizerId = null)
        {
            ...
            if (organizerId.HasValue && seminar.OrganizerId != organizerId.Value) return null;
        ...
        public async Task<SeminarResponse?> GetByIdForViewerAsync(int seminarId, int currentUserId)
        {
            ...
            if (seminar.OrganizerId == currentUserId) return _mapper.Map<SeminarResponse>(seminar);
        ...
        public async Task<SeminarResponse> CreateAsync(int organizerId, SeminarCreateRequest request, CancellationToken cancellationToken = default)
        {
            ...
            if (request.MaxParticipants.HasValue && request.MaxParticipants.Value > 0 && normalizedGuestEmails.Count > request.MaxParticipants.Value)
            ...
            if (participants.Count > 0)
            {
                ...
                if (invitationTimestampChanged) await _participantRepository.SaveChangesAsync();
            }
        ...
        {
            var seminar = await _seminarRepository.GetByIdWithParticipantsAsync(seminarId);
            if (seminar == null || seminar.OrganizerId != organizerId) return null;
            ...
            if (request.StartTime.HasValue)
            {
                ...
                seminar.IsReminderSent = false;
                seminar.ReminderSentAt = null;
                foreach (var participant in seminar.SeminarParticipants) participant.EventReminderSentAt = null;
            }

            if (request.EndTime.HasValue) seminar.EndTime = request.EndTime.Value;
            if (request.Content != null) seminar.Content = request.Content.Trim();
            if (request.MaxParticipants.HasValue) seminar.MaxParticipants = request.MaxParticipants.Value;
        ...
        {
            var seminar = await _seminarRepository.GetByIdWithParticipantsAsync(seminarId);
            if (seminar == null || seminar.OrganizerId != organizerId) return false;
        ...
        public async Task<SeminarInviteResponse> InviteAsync(int seminarId, int organizerId, SeminarInviteRequest request, CancellationToken cancellationToken = default)
        {
            ...
            foreach (var email in normalizedEmails)
            {
                ...
                if (existing != null)
                {
                    if (existing.InvitationSentAt == null) participantsToSend.Add(existing);
        ...
        {
            if (emails == null) return new List<string>();
            ...
            foreach (var rawEmail in emails)
            {
                ...
                if (seen.Add(email)) normalized.Add(email);
            }
        ...
        {
            if (endTime <= nowUtc) return "Completed";
            if (startTime <= nowUtc) return "In Progress";
            return "Upcoming";
        }
